...
Yii::import('ext.picms.widgets.datagrid.CmsDataGrid');


class CQTreeGridView extends CmsDataGrid
{
    ...
    public function registerClientScript()
    {
        ...
        $cs->registerScript('treeTable', '
            $(document).ready(function()  {
              $("#'.$this->getId().' .table").treeTable();
            });
            ');

        $cs->registerScript('draganddrop', '
            $(document).ready(function()  {
               $("#'.$this->getId().' .table tr.initialized").draggable({
                  helper: "clone",
                  opacity: .75,
                ...
                $("#'.$this->getId().' .table tr.initialized, #'.$this->getId().' .table tr.before, #'.$this->getId().' .table tr.after").droppable({
                    accept: ".initialized",
                    drop: function(e, ui) {
