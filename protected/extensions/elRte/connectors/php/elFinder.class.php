...
class elFinder {
	...
	public function __construct($options=array()) {
		...
		if (substr($this->_options['root'], -1) == '/') {
		...
		$this->_fakeRoot = !$this->_options['rootAlias'] 
			? $this->_options['root'] 
			: dirname($this->_options['root']).'/'.$this->_options['rootAlias'];
		...
		if ($this->_options['tmbDir']) {
			$tmbDir = $this->_options['root'].'/'.$this->_options['tmbDir'];
			...
			if ($this->_options['tmbDir']) {
				...
				if (rand(1, 200) <= $this->_options['tmbCleanProb']) {
					...
					for ($i=0, $s = count($ls); $i < $s; $i++) { 
						if ('.' != $ls[$i] && '..' != $ls[$i]) {
							@unlink($this->_options['tmbDir'].'/'.$ls[$i]);
						}
					}
				}
			}
	...
	protected function _rename()
	{
		...
		} elseif (!rename($target, $dir.'/'.$name)) {
			$this->_result['error'] = 'Unable to rename file';
		} else {
			$this->_rmTmb($target);
			$this->_logContext['from'] = $target;
			$this->_logContext['to']   = $dir.'/'.$name;
			$this->_result['select']   = array($this->_hash($dir.'/'.$name));
			$this->_content($dir, is_dir($dir.'/'.$name));
		}
	}
	...
	protected function _mkdir()
	{
		...
		$this->_logContext['dir'] = $dir.'/'.$_GET['name'];
		if (!$this->_isAllowed($dir, 'write')) {
			...
			$this->_result['error'] = 'File or folder with the same name already exists';
		} elseif (!@mkdir($dir.'/'.$name, $this->_options['dirMode'])) {
			$this->_result['error'] = 'Unable to create folder';
		} else {
			$this->_logContext['dir'] = $dir.'/'.$name;
			$this->_result['select']  = array($this->_hash($dir.'/'.$name));
	...
	protected function _mkfile()
	{
		...
		$this->_logContext['file'] = $dir.'/'.$_GET['name'];
		if (!$this->_isAllowed($dir, 'write')) {
			$this->_result['error'] = 'Access denied';
		} elseif (false == ($name = $this->_checkName($_GET['name'])) ) {
			$this->_result['error'] = 'Invalid name';
	...
	protected function _cwd($path)
	{
		...
		if ($path == $this->_options['root']) {
			...
			$name = basename($path);
			$rel .= '/'.substr($path, strlen($this->_options['root'])+1);
		}
				...
				if (@copy($img, $tmb)) {
					...
					if (file_exists($tmb)) {
						...
						if ($mime == 'image/vnd.adobe.photoshop' || $mime = 'image/gif') {
							$pinfo = pathinfo($tmb);
							$test = $pinfo['dirname'].'/'.$pinfo['filename'].'-0.'.$pinfo['extension'];
							if (file_exists($test)) {
	...
	protected function _tmbPath($path)
	{
		...
		if ($this->_options['tmbDir']) {
			$tmb = dirname($path) != $this->_options['tmbDir']
				? $this->_options['tmbDir'].'/'.$this->_hash($path).'.png'
				: $path;
		}
		return $tmb;
	}
	...
	protected function _path2url($path)
	{
		...
		return $this->_options['URL'].($dir ? str_replace('/', '/', $dir).'/' : '').$file;
	}
