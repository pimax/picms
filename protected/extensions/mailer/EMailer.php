...
class EMailer
{
   ...
   public function SetHtmlBody($view, $params = array(), $layout = 'default')
   {
        $body = Yii::app()->controller->renderPartial($this->pathViews.'.'.$view, array_merge($params, array('content'=>$this->_myMailer)), true);
        
        return $this->_myMailer->MsgHTML(Yii::app()->controller->renderPartial($this->pathLayouts.'.'.$layout, array('content'=>$body), true));
   }
}