...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver {
	...
	protected function configure() {
		...
		if ($this->options['tmbPath']) {
			$this->options['tmbPath'] = strpos($this->options['tmbPath'], '/') === false
				// tmb path set as dirname under root dir
				? $this->root.'/'.$this->options['tmbPath']
		...
		if ($this->attr($this->root, 'read') && !$this->tmbURL && $this->URL) {
			if (strpos($this->tmbPath, $this->root) === 0) {
				$this->tmbURL = $this->URL.str_replace('/', '/', substr($this->tmbPath, strlen($this->root)+1));
	...
	protected function _readlink($path) {
		...
		if ($this->_inpath($atarget, $this->aroot)) {
			return $this->_normpath($this->root.'/'.substr($atarget, strlen($this->aroot)+1));
		}
		// echo $path.'<br>';
		return false;
	}
	...
	protected function _symlink($target, $path, $name='') {
		...
		return @symlink('.'.'/'.$this->_relpath($target), $path.'/'.$name);
	}
	...
	protected function _copy($source, $targetDir, $name='') {
		$target = $targetDir.'/'.($name ? $name : basename($source));
