...
abstract class elFinderVolumeDriver {
	...
	public function mount(array $opts) {
		...
		$this->separator = isset($this->options['separator']) ? $this->options['separator'] : '/';
	...
	protected function decode($hash) {
		if (strpos($hash, $this->id) === 0) {
			...
			return $this->_abspath($path);//$this->root.($path == '/' ? '' : '/'.$path); 
		}
	}
	...
	protected function gettmb($path) {
		if ($this->tmbURL && $this->tmbPath) {
			...
			if (file_exists($this->tmbPath.'/'.$name)) {
				return $name;
			}
		}
		return false;
	...
	protected function rmTmb($path) {
		$path = $this->tmbPath.'/'.$this->tmbName($path);
		if (file_exists($path)) {
			@unlink($path);
		}
	}
